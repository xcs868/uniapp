<template>
	<view class="grace-dialog-shade" v-if="show" @tap.stop="closeDialog" @touchmove.stop="stopFun" :style="{backgroundColor:background}">
		<view class="grace-dialog gdFadeIn" @tap.stop="stopFun" :style="{width:width, borderRadius:borderRadius}">
			<view class="grace-dialog-title" v-if="isTitle" :style="{fontSize:titleSize, color:titleColor, fontWeight:titleWeight?'bold':'', height:titleHeight+'rpx'}">{{title}}</view>
...
.grace-dialog{width:580rpx; background:#FFFFFF; position:fixed;bottom: 0;}
